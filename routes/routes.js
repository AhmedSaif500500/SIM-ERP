...
router.get('/register_ar', (req, res) => {
    res.sendFile(path.join(__dirname, '..', 'views' , 'ar', 'register', 'register_ar.html'));
});


router.get('/history_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission === 6 || req.session.history_permission > 0) {
...
router.get('/general_settings_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission === 6 || req.session.bread_permission > 0) { //معلق
...
router.get('/permissions_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission === 6) {
...
router.get('/users_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {        
        if (req.session.is_owner_permission) {            
...
router.get('/hr_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 ||  req.session.employees_permission > 0 || req.session.departments_permission > 0 || req.session.effects_permission > 0) {
...
router.get('/employees_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 ||  req.session.employees_permission > 0) {
...
router.get('/employees_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 2 ||  req.session.employees_permission > 1) {
...
router.get('/effects_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 2 ||  req.session.effects_permission > 1) {
...
router.get('/departments_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 ||  req.session.departments_permission > 0) {
...
router.get('/departments_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 2 ||  req.session.departments_permission > 1) {
...
router.get('/salesMain_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.salesman_permission > 0 || req.session.sales_qutation_permission > 0 || req.session.sales_order_permission > 0 || req.session.sales_invoice_permission > 0) { // 
...
router.get('/salesman_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.salesman_permission > 0) {
...
router.get('/sales_qutation_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.sales_qutation_permission > 0) {
...
router.get('/sales_qutation_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.sales_qutation_permission > 1) {
...
router.get('/sales_qutation_update_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.sales_qutation_permission > 3) {
...
router.get('/sales_order_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.sales_order_permission > 0) {
...
router.get('/sales_order_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.sales_order_permission > 1) {
...
router.get('/sales_order_update_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.sales_order_permission > 3) {
...
router.get('/sales_invoice_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.sales_invoice_permission > 0) {
...
router.get('/sales_invoice_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.sales_invoice_permission > 1) {
...
router.get('/sales_invoice_update_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.sales_invoice_permission > 3) {
...
router.get('/purshasesMain_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.purshases_qutation_permission > 0 || req.session.purshases_order_permission > 0 || req.session.purshases_invoice_permission > 0) {
...
//     if (req.session.isLoggedIn) {
//         if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.purshasesman_permission > 0) {
...
router.get('/purshases_qutation_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.purshases_qutation_permission > 0) {
...
router.get('/purshases_qutation_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.purshases_qutation_permission > 1) {
...
router.get('/purshases_qutation_update_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.purshases_qutation_permission > 3) {
...
router.get('/purshases_order_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.purshases_order_permission > 0) {
...
router.get('/purshases_order_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.purshases_order_permission > 1) {
...
router.get('/purshases_order_update_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.purshases_order_permission > 3) {
...
router.get('/purshases_invoice_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.purshases_invoice_permission > 0) {
...
router.get('/purshases_invoice_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.purshases_invoice_permission > 1) {
...
router.get('/purshases_invoice_update_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.purshases_invoice_permission > 3) {
...
router.get('/customers_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 ||  req.session.customers_permission > 1) {
...
router.get('/import_data_customers', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 ||  req.session.customers_permission > 1) {
            res.sendFile(path.join(__dirname, '..', 'views', 'ar' , 'customers' ,'import_data_customers.html'));
        }else{
            res.redirect('/customers_view_ar?reason=0');
        };
    } else {        
...
router.get('/production_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 ||  req.session.production_permission > 0) {
...
router.get('/production_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 2 ||  req.session.production_permission > 1) {
...
router.get('/bread_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 ||  req.session.bread_permission > 0) {
...
router.get('/bread_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 2 ||  req.session.bread_permission > 1) {
...
router.get('/accounts_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 2 ||  req.session.accounts_permission > 1) {
...
router.get('/itemsMain_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 ||  req.session.items_permission > 0 || req.session.itemsLocations_permission > 0) {
...
router.get('/itemsLocations_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 ||  req.session.itemsLocations_permission > 0) {
...
router.get('/itemsLocations_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 2 ||  req.session.itemsLocations_permission > 1) {
...
router.get('/items_transfer_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 2 ||  req.session.items_transfer_permission > 0) {
...
router.get('/items_transfer_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 2 ||  req.session.items_transfer_permission > 1) {
...
router.get('/items_transfer_update_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 ||  req.session.items_transfer_permission > 0) {
...
router.get('/items_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 ||  req.session.items_permission > 0) {
...
router.get('/productionMain_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 ||  req.session.items_permission > 0 || req.session.production_permission > 0) {
...
router.get('/transaction_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 ||  req.session.transaction_permission > 0) {
...
router.get('/transaction_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 2 ||  req.session.transaction_permission > 1) {
            res.sendFile(path.join(__dirname, '..', 'views' , 'ar', 'accounts', 'transactions', 'transaction_add_ar.html'));
        }else{
...
router.get('/settings_taxes_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1) {
...
router.get('/settings_taxes_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 2) {
...
router.get('/services_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.services_permission > 0) {
...
router.get('/services_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 2 || req.session.services_permission > 1) {
...
router.get('/sales_returns_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.sales_permission > 0) {
...
router.get('/sales_returns_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.sales_permission > 1) {
...
router.get('/sales_returns_update_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.sales_permission > 3) {
...
router.get('/purshases_returns_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.purshases_permission > 0) {
...
router.get('/purshases_returns_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.purshases_permission > 1) {
...
router.get('/purshases_returns_update_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.purshases_permission > 3) {
...
router.get('/fixedAssestsMain_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.fixed_assests_permission > 0 || req.session.accumulated_depreciation_permission > 0 || req.session.disposed_fixed_asset_permission > 0) {
...
router.get('/accumulated_depreciation_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.accumulated_depreciation_permission > 0) {
...
router.get('/accumulated_depreciation_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.accumulated_depreciation_permission > 1) {
...
router.get('/disposed_fixed_asset_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.disposed_fixed_asset_permission > 0) {
...
router.get('/disposed_fixed_asset_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.disposed_fixed_asset_permission > 1) {
...
router.get('/cashMain_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.fixed_assests_permission > 0 || req.session.cash_accounts_permission > 0 || req.session.cash_accounts_permission_permission > 0) {  //! معلق
...
router.get('/cash_accounts_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.cash_accounts_permission > 0) {
...
router.get('/cash_accounts_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.cash_accounts_permission > 1) {
...
router.get('/cash_rc_view_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.cash_transaction_permission > 0) {
...
router.get('/cash_rc_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 1 || req.session.cash_transaction_permission > 1) {
...
router.get('/cash_transfer_add_ar', (req, res) => {
    if (req.session.isLoggedIn) {
        if (req.session.is_owner_permission || req.session.general_permission > 2 || req.session.cash_transaction_permission > 1) {
