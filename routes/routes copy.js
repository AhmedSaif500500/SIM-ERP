...
const db = require('../server'); // استيراد الاتصال بقاعدة البيانات
...
const checkOwner = async (req, res, next) => {
    ...
    try {
      // التحقق من وجود المالك في قاعدة البيانات
      const ownerData = await db.oneOrNone("SELECT id FROM owners WHERE owner_name = $1", [owner]);
  
      if (!ownerData) {
        return res.status(404).send("المالك غير موجود!");
      }
  
      // حفظ owner_id في الجلسة لاستخدامه لاحقًا
...
router.get("/:owner/notes_ar", checkOwner, async (req, res) => {
    if (req.session.isLoggedIn) {  
        res.sendFile(path.join(__dirname, "..", "views", "ar", "notes_ar.html"));
    } else {
        res.redirect(`/${req.params.owner}/login`);
    }
});
