...
back_href.onclick = async function (event) {
    ...
    await back_href_fn1(getData_fn, `accumulated_depreciation_viewArray`, `accumulated_depreciation_view_ar`, `fixedAssestsMain_view_ar`)
};
...
async function Execution() {
    try {
        ...
        permissionName = 'accumulated_depreciation_permission'
        start_date = f0_input_start_date1.value
        end_date = f0_input_end_date1.value
        Qkey = null
        back_href_page = 'accumulated_depreciation_view_ar'
        back_title_page = 'إهلاك الاصول الثابتة'
        ...
        backUp_page1(`accumulated_depreciation_viewArray`, Qkey, permissionName, start_date, end_date, back_href_page, back_title_page)
    } catch (error) {
        catch_error(error);
    } finally {
...
async function table_view_btn(updateBtn) {
    ...
    const accumulated_depreciation_update_data = {
        ...
        href_pageName : `accumulated_depreciation_view_ar`,
        href_pageTitle : 'إهلاك الأصول الثابتة',
        //addmore
    };
...
document.addEventListener("DOMContentLoaded", async function () {
    try {
        ...
        if (conditionsArray.length === 0){
            ...
            back_title_page = 'إدارة الأصول الثابتة'
    
            pagePermission("view", permissionName);  // معلق
            sessionStorage.removeItem('accumulated_depreciation_viewArray');
            backUp_page1(`accumulated_depreciation_viewArray`, Qkey, permissionName, start_date, end_date, back_href_page, back_title_page)
            await restore_page1(getData_fn, `accumulated_depreciation_viewArray`)
        } else {
            await restore_page1(getData_fn, `accumulated_depreciation_viewArray`)
        }
