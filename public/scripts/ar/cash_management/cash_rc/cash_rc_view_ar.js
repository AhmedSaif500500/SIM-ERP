...
function backUp_filter_div_conditions() {
    ...
    const conditionsArray = JSON.parse(sessionStorage.getItem('cash_rc_ViewArray')) || [];

    // إضافة الكائن الجديد إلى المصفوفة
    conditionsArray.push(conditions);

    // حفظ المصفوفة المحدثة في sessionStorage
    sessionStorage.setItem('cash_rc_ViewArray', JSON.stringify(conditionsArray));
}
...
back_href.onclick = async function (event) {
    ...
    const array = JSON.parse(sessionStorage.getItem(`cash_rc_ViewArray`)) || [];
...
function CheckUrlParams_transaction_update_ar() {
    try {
        ...
        if (!urlData || urlData.pageName !== "cash_rc_update_ar") {
            return true;
        }
        if (urlData !== "noParams") {
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    await getData_fn();
    const conditionsArray = sessionStorage.getItem(`cash_rc_ViewArray`);
