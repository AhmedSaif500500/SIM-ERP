...
//pagePermission("view", "cash_transaction_permission");
...
newBtn.onclick = function (){
    ...
    window.location.href = "/cash_rc_add_ar";
  }
...
back_href.onclick = async function (event) {
    ...
    await back_href_fn1(getData_fn, `cash_rc_viewArray`, `cash_rc_view_ar`, `cashMain_view_ar`)
};
...
function deafult_checkbox() {
    ...
    call_default_checkbox('f2',true,true,false) // name
    call_default_checkbox('f3',true,false,false) // note
    call_default_checkbox('f4',true,true,false) // total_value
...
async function Execution() {
    try {
        ...
        Qkey = null
        back_href_page = 'cash_rc_view_ar'
        back_title_page = 'سندات القبض'
        ...
        backUp_page1(`cash_rc_viewArray`, Qkey, permissionName, start_date, end_date, back_href_page, back_title_page)
    } catch (error) {
        catch_error(error);
    } finally {
...
function fillTable() {
    try {
        ...
        let style_referenceconcat = `display:${f1_checkbox.checked ? "table-cell" : "none"}; width: ${f2_checkbox.checked ? "auto" : "100%"}; white-space: nowrap; text-align: start`;
        let style_account_name = `display:${f2_checkbox.checked ? "table-cell" : "none"}; width: ${f3_checkbox.checked ? "auto" : "100%"}; white-space: wrap; text-align: start;`;
        let style_note = `display:${f3_checkbox.checked ? "table-cell" : "none"}; min-width: 25rem; width: 100%; white-space: wrap; text-align: start;`;
        let style_total_value = `display:${f4_checkbox.checked ? "table-cell" : "none" }; width: auto; white-space: nowrap; text-align: start`;
        ...
        let tableHTML = `<table id="review_table" class="review_table">
                        <thead>
                            <tr>
                                ...
                                <th style="${style_account_name}">حساب القبض</th>
...
function performSearch() {
    try {
        ...
        array1 = filteredData_Array.filter((row) => {
            ...
            const account_name_Match = performSearch_Row(f2_checkbox,"referenceconcat",searchValue,row);
            const noteMatch = performSearch_Row(f3_checkbox,"general_note",searchValue,row);
            const totalValueMatch = performSearch_Row(f4_checkbox,"total_value",searchValue,row);
...
async function table_view_btn_fn(viewBtn) {
    ...
    const permission = await btn_permission("cash_transaction_permission", "view"); // معلق
    ...
    const cash_rc_update_data = {
        x: row.querySelector(`.td_id`).textContent,
        href_pageName : `cash_rc_view_ar`,
        href_pageTitle : 'سندات القبض',
    };

    sessionStorage.removeItem('cash_rc_update_data')
    sessionStorage.setItem('cash_rc_update_data', JSON.stringify(cash_rc_update_data));                            
...
document.addEventListener("DOMContentLoaded", async function () {
    try {
        ...
        let conditionsArray = JSON.parse(sessionStorage.getItem("cash_rc_viewArray")) || [];
        if (conditionsArray.length === 0){
            ...
            sessionStorage.removeItem('cash_rc_viewArray');
            backUp_page1(`cash_rc_viewArray`, Qkey, permissionName, start_date, end_date, back_href_page, back_title_page)
            await restore_page1(getData_fn, `cash_rc_viewArray`)
        } else {
            await restore_page1(getData_fn, `cash_rc_viewArray`)
        }
