    ...
    // 4 : text-align: center / left / right / justify   da 3ashan tet7km fe el text ymen wala shemal wala fe ele nos
        ...
        slice_Array1.forEach(row => {
            tableHTML += `<tr>
                            ...
                            <td style="width: auto; white-space: nowrap;">${row.procution_amount}</td>
                            <td style="width: auto; white-space: nowrap;">${row.sales_amount}</td>
...
let tableBody = document.getElementById("production_table").getElementsByTagName("tbody")[0];
...
for (let i = 0; i < tableBody.rows.length; i++) {
    ...
    let production_cell = row.cells[4]; let production_Value =  parseFloat(production_cell.textContent);
    let sales_cell = row.cells[5]; let sales_value =  parseFloat(sales_cell.textContent);
  

    // التحقق من أن القيم قابلة للتحويل إلى أرقام وجمعها
    if (!isNaN(production_Value)) {
        total_production += production_Value;
    }

    if (!isNaN(sales_value)) {
...
document.getElementById("tfooter5").textContent = total_production;
document.getElementById("tfooter6").textContent = total_sales;
