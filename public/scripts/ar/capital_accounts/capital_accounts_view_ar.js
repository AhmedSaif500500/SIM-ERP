...
newBtn.onclick = function (){
    ...
    window.location.href = "/capital_accounts_add_ar";
  }
...
async function getData_fn() {
    try {
        ...
        h2_text_div.textContent = `حسابات رأس المال`
...
function fillTable() {
    try {
        ...
        let fn1 = `onclick = "table_balance1_btn_to_statetment_fn1(this, 'td_id', 'accounts_permission', firstDayOfYear, end_date, 'capital_accounts_view_ar', 'حسابات رأس المال', false, 'account_statement_view_ar', 'obj_statement')"`;
...
async function table_update_btn_fn(updateBtn) {
    ...
    const permission = await btn_permission("accounts_permission", "view");
