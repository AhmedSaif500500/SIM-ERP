...
const indices = [0, 100, 1, 3, 4, 5, 6, 7, 8, 9]; // ضع هنا الأرقام التي تريد تضمينها


back_href.onclick = async function (event) {
    event.preventDefault();
    await back_href_fn1(getData_fn, `purshases_invoice_viewArray`, `purshases_invoice_view_ar`, `purshasesMain_view_ar`)
};
...
async function Execution() {
    try {
        ...
        Qkey = null
        back_href_page = 'purshases_invoice_view_ar'
        back_title_page = 'فواتير مشتريات'
        ...
        backUp_page1(`purshases_invoice_viewArray`, Qkey, permissionName, start_date, end_date, back_href_page, back_title_page)
...
async function table_view_btn_fn(updateBtn) {
    ...
    const purshases_invoice_update_data = {
        x: row.querySelector(`.td_id`).textContent,
        href_pageName : `purshases_invoice_view_ar`,
        href_pageTitle : 'فواتير المشتريات',
    ...
    sessionStorage.setItem('purshases_invoice_update_data', JSON.stringify(purshases_invoice_update_data));                            
...
document.addEventListener("DOMContentLoaded", async function () {
    try {
        ...
        if (conditionsArray.length === 0){
            ...
            sessionStorage.removeItem('purshases_invoice_viewArray');
            backUp_page1(`purshases_invoice_viewArray`, Qkey, permissionName, start_date, end_date, back_href_page, back_title_page)
            await restore_page1(getData_fn, `purshases_invoice_viewArray`)
        } else {
            await restore_page1(getData_fn, `purshases_invoice_viewArray`)
        }
