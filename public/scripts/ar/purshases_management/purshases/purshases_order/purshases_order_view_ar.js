...
back_href.onclick = async function (event) {
    ...
    await back_href_fn1(getData_fn, `purshases_order_viewArray`, `purshases_order_view_ar`, `purshasesMain_view_ar`)
};
...
async function Execution() {
    try {
        ...
        permissionName = 'purshases_order_permission'
        start_date = f0_input_start_date1.value
        end_date = f0_input_end_date1.value
        Qkey = null
        back_href_page = 'purshases_order_view_ar'
        back_title_page = 'أوامر المشتريات'
        ...
        backUp_page1(`purshases_order_viewArray`, Qkey, permissionName, start_date, end_date, back_href_page, back_title_page)
    } catch (error) {
        catch_error(error);
    } finally {
...
async function table_update_btn_fn(updateBtn) {
    ...
    const purshases_qutation_update_data = {
        ...
        href_pageName : `purshases_order_view_ar`,
        href_pageTitle : 'أوامر المشتريات',
...
document.addEventListener("DOMContentLoaded", async function () {
    try {
        ...
        if (conditionsArray.length === 0){
            ...
            sessionStorage.removeItem('purshases_order_viewArray');
            backUp_page1(`purshases_order_viewArray`, Qkey, permissionName, start_date, end_date, back_href_page, back_title_page)
            await restore_page1(getData_fn, `purshases_order_viewArray`)
        } else {
            await restore_page1(getData_fn, `purshases_order_viewArray`)
        }
