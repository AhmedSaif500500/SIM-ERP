...
pagePermission("add","purshases_qutation_permission"); // معلق
...
const is_RowTax  = is_RowTax_checkBox.checked
  ...
  if (tableRows.length > 0) { // التأكد من وجود بيانات داخل المصفوفه اولا
    ...
    const posted_Obj = {vendorId, total, datex,itemLocationId, is_RowNote, is_RowDiscount, is_RowTax, general_note, posted_array}
    ...
    if (post){
      sessionStorage.removeItem('purshases_qutation_ViewArray') // معلق
    }
...
async function get_Data_for_add_page_fn() {
  ...
  data_accounts = await new_fetchData_postAndGet(
    ...
    {},
    'purshases_qutation_permission', 'add', // معلق
    15,
    false,false,
    true,
    false,false,
