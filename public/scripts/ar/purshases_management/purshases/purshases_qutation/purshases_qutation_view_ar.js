...
function backUp_filter_div_conditions() {
    ...
    const conditionsArray = JSON.parse(sessionStorage.getItem('purshases_qutation_ViewArray')) || [];

    // إضافة الكائن الجديد إلى المصفوفة
    conditionsArray.push(conditions);

    // حفظ المصفوفة المحدثة في sessionStorage
    sessionStorage.setItem('purshases_qutation_ViewArray', JSON.stringify(conditionsArray));
}
...
back_href.onclick = async function (event) {
    ...
    const array = JSON.parse(sessionStorage.getItem(`purshases_qutation_ViewArray`)) || [];
...
function CheckUrlParams_transaction_update_ar() {
    try {
        const urlData = getURLData(
            "data",
            "purshases_qutation_view_ar",
            "رابط غير صالح : سيتم اعادة توجيهك الى صفحة عرض اسعار المشتريات"
        );

        if (!urlData || urlData.pageName !== "purshases_qutation_update_ar") {
            return true;
        }
        if (urlData !== "noParams") {
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    await getData_fn();
    const conditionsArray = sessionStorage.getItem(`purshases_qutation_ViewArray`);
