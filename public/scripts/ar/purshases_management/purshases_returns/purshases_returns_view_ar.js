...
pagePermission("view", "purshases_returns_permission");  // معلق
...
const indices = [0, 1, 3, 4, 5, 7]; // ضع هنا الأرقام التي تريد تضمينها
...
function backUp_filter_div_conditions() {
    ...
    const conditionsArray = JSON.parse(sessionStorage.getItem('purshases_returns_Array')) || [];

    // إضافة الكائن الجديد إلى المصفوفة
    conditionsArray.push(conditions);

    // حفظ المصفوفة المحدثة في sessionStorage
    sessionStorage.setItem('purshases_returns_Array', JSON.stringify(conditionsArray));
}
...
back_href.onclick = async function (event) {
    ...
    const array = JSON.parse(sessionStorage.getItem(`purshases_returns_Array`)) || [];
...
function restore_filter_div_conditions(NUM_ektp_rakm_el_restore_elyEnta3ayzTerg3oMnel2a5er_maslan_1_ya3nyLastRestore) {
    ...
    // استرجاع المصفوفة المحفوظة من sessionStorage
    let conditionsArray = JSON.parse(sessionStorage.getItem("purshases_returns_Array")) || [];
...
async function filter_icon_cancel_fn() {
    try {
        ...
        if(is_filter){
            ...
            await getData_fn();
            closeDialog();
            sessionStorage.removeItem('purshases_returns_Array');
            conditionsArray = []
...
async function getData_fn() {
    try {
        ...
        data = await new_fetchData_postAndGet(
            "/get_purshases_returns_Data_view",
            {start_date, end_date},
            "purshases_returns_permission","view",
            15,
            false,'',
            false,
            true,content_space,
...
function fillTable() {
    try {
        ...
        let style_invoice_reference = `display:${f7_checkbox.checked ? "table-cell" : "none"}; width: auto; white-space: nowrap; text-align: start`;
        ...
        // let is_returnsd = `display:${f6_checkbox.checked ? "table-cell" : "none" }; width: auto; white-space: nowrap; text-align: start`;
        ...
        let tableHTML = `<table id="review_table" class="review_table">
                        <thead>
                            <tr>
                                ...
                                <th style="${style_id}">invoice_id</th>
                                <th style="${style_invoice_reference}">فاتورة مشتريات</th>
        ...
        slice_array1.forEach((row) => {
            ...
            // let returnsdstatusClass;
            
            // if(row.is_returnsd === 'مفوتر'){
            //     returnsdstatusClass = 'table_green_condition'
            // }else if(row.is_returnsd === 'غير مفوتر'){
            //     returnsdstatusClass = 'table_orange_condition'
            // }
            ...
            tableHTML +=
                     `<tr>
                        ...
                        <td style="${style_id}" class="td_invoice_id">${row.invoice_id}</td>
                        <td style="${style_invoice_reference}" class="td_invoice_reference">${row.invoice_reference}</td>
        ...
        tableHTML += `
                    <tr class="table_totals_row">
                        ...
                        <td id="footer_style_invoice_id" style="${style_id}"></td>
                        <td id="footer_style_invoice_reference" style="${style_invoice_reference}"></td>
...
function performSearch() {
    try {
        ...
        array1 = filteredData_Array.filter((row) => {
            ...
            const invoiceReference = performSearch_Row(f7_checkbox,"invoice_reference",searchValue,row);
...
async function table_view_btn_fn(updateBtn) {
    ...
    const permission = await btn_permission("purshases_returns_permission", "view");
    ...
    const purshases_returns_update_data = {
        ...
        invoice_id: row.querySelector(`.td_invoice_id`).textContent,
...
function CheckUrlParams_purshasesreturns_update_ar() {
    try {
        const urlData = getURLData(
            "data",
            "purshases_returns_view_ar",
            "رابط غير صالح : سيتم اعادة توجيهك الى صفحة مرتجع المشتريات"
        );

        if (!urlData || urlData.pageName !== "purshases_returns_update_ar") {
            return true;
        }
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    const result2 = CheckUrlParams_purshasesreturns_update_ar();
    if (!result2) {
        return;
    }

    await getData_fn();
    const conditionsArray = sessionStorage.getItem(`purshases_returns_Array`);
