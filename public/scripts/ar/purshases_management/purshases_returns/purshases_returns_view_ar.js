...
back_href.onclick = async function (event) {
    ...
    await back_href_fn1(getData_fn, `purshases_returns_viewArray`, `purshases_returns_view_ar`, `purshasesMain_view_ar`)
};
...
async function Execution() {
    try {
        ...
        permissionName = 'purshases_returns_permission'
        start_date = f0_input_start_date1.value
        end_date = f0_input_end_date1.value
        Qkey = null
        back_href_page = 'purshases_returns_view_ar'
        back_title_page = 'مرتجعات المشتريات'
        ...
        backUp_page1(`purshases_returns_viewArray`, Qkey, permissionName, start_date, end_date, back_href_page, back_title_page)
    } catch (error) {
        catch_error(error);
    } finally {
...
async function table_view_btn_fn(updateBtn) {
    ...
    const purshases_returns_update_data = {
        ...
        href_pageName : `purshases_returns_view_ar`,
        href_pageTitle : 'مرتجعات المشتريات',
...
document.addEventListener("DOMContentLoaded", async function () {
    try {
        ...
        let conditionsArray = JSON.parse(sessionStorage.getItem("purshases_returns_viewArray")) || [];
        if (conditionsArray.length === 0){
            ...
            backUp_page1(`purshases_returns_viewArray`, Qkey, permissionName, start_date, end_date, back_href_page, back_title_page)
            await restore_page1(getData_fn, `purshases_returns_viewArray`)
        } else {
            await restore_page1(getData_fn, `purshases_returns_viewArray`)
        }
