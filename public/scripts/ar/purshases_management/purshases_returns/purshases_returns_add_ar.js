...
pagePermission("add","purshases_returns_permission");
...
const is_RowTax  = is_RowTax_checkBox.checked
  ...
  if (tableRows.length > 0) { // التأكد من وجود بيانات داخل المصفوفه اولا
    ...
    const posted_Obj = {vendorId, total, datex, invoiceReferenceId, itemLocationId, is_RowNote, is_RowDiscount, is_RowTax, general_note, location_name, posted_array}
...
async function get_Data_for_add_page_fn() {
  ...
  data_accounts = await new_fetchData_postAndGet(
    "/get_data_for_purshases_returns_add",
    {},
    'purshases_returns_permission', 'add',
    15,
    false,false,
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  const convertToreturns = JSON.parse(sessionStorage.getItem('purshases_returns_update_data'));
  
  
  if (convertToreturns && convertToreturns.invoiceToReturns){
    await showpurshasesreturnsData(convertToreturns.x, convertToreturns.x, convertToreturns.vendor_id, 'invoice')
    const obj = {pageName : 'purshases_returns_add_ar', x : convertToreturns.x}
    sessionStorage.setItem('purshases_invoice_update_data', JSON.stringify(obj));                            
    back_href.href = `purshases_invoice_update_ar`
  }else{
    ...
    purshasesInvoiceReferencesArray =  Data.purshasesInvoiceReferencesArray;
      ...
      create_drop_down_with_External_DataArray(`dropdown_div4`,purshasesInvoiceReferencesArray)
