...
//#region get purshasesman and itemsLocations
...
// let purshasesmanArray = [];
let itemslocationsArray = [];
let purshasesInvoiceReferencesArray = [];

async function showpurshasesreturnsData(x, invoiceId, vendor_id, type){

  try {
    let permType
    let url;
    if (type === 'invoice'){
      url = "/get_data_for_purshasesInvoiceToreturns"
      permType = 'add'
    }else{
      url = "/get_data_for_purshases_returns_update"
      permType = 'update'
    }
  ...
  Data = await new_fetchData_postAndGet(
    url,
    {x, invoiceId, vendor_id},
    'purshases_returns_permission', permType, // معلق
    15,
    false,false,
    true,
    false,false,
    false,false,
    false,false,false,
    false,"purshases_returns_view_ar",
  ...
  headerDataArray = Data.headerData;
  ...
  purshasesInvoiceReferencesArray =  Data.purshasesInvoiceReferencesArray;

    if (!Data || !itemsDataArray || !itemsDataArray || !bodyDataArray){
      await redirection('purshases_order_view_ar','fail','حدث خطأ اثتاء معالجه البيانات')
      return
    }
    ...
    //! show header data 
     ...
     const invoice_Id = type === 'invoice' ? +headerDataArray.id : +headerDataArray.invoice_id;  
    ...
    create_drop_down_with_External_DataArray(`dropdown_div4`,purshasesInvoiceReferencesArray); selectedRow_dropdownDiv(`dropdown_div4`,purshasesInvoiceReferencesArray,invoice_Id);
    
  

    //let status = headerDataArray.is_returnsd; 
    // if(status){
