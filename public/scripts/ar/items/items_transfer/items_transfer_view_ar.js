...
back_href.onclick = async function (event) {
    ...
    await back_href_fn1(getData_fn, `items_transfer_viewArray`, `items_transfer_view_ar`, `itemsMain_view_ar`)
};
...
async function Execution() {
    try {
        ...
        permissionName = 'items_transfer_permission'
        start_date = f0_input_start_date1.value
        end_date = f0_input_end_date1.value
        Qkey = null
        back_href_page = 'items_transfer_view_ar'
        back_title_page = 'تحويلات بين مواقع المخزون'
        ...
        backUp_page1(`items_transfer_viewArray`, Qkey, permissionName, start_date, end_date, back_href_page, back_title_page)
...
async function table_view_btn(updateBtn) {
    ...
    const items_transfer_update_data = {
        ...
        href_pageName : `items_transfer_view_ar`,
        href_pageTitle : 'تحويلات بين مواقع المخزون',
    };
...
document.addEventListener("DOMContentLoaded", async function () {
    try {
        ...
        if (conditionsArray.length === 0){
            ...
            sessionStorage.removeItem('items_transfer_viewArray');
            backUp_page1(`items_transfer_viewArray`, Qkey, permissionName, start_date, end_date, back_href_page, back_title_page)
            await restore_page1(getData_fn, `items_transfer_viewArray`)
        } else {
            await restore_page1(getData_fn, `items_transfer_viewArray`)
        }
