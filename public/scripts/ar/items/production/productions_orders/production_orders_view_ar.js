...
back_href.onclick = async function (event) {
    ...
    await back_href_fn1(getData_fn, `production_orders_viewArray`, `production_orders_view_ar`, `productionMain_view_ar`)
};
...
async function Execution() {
    try {
        ...
        Qkey = null
        back_href_page = 'production_orders_view_ar'
        back_title_page = 'أوامر التصنيع'
        ...
        backUp_page1(`production_orders_viewArray`, Qkey, permissionName, start_date, end_date, back_href_page, back_title_page)
    } catch (error) {
        catch_error(error);
    } finally {
...
async function table_view_btn_fn(viewBtn) {
    ...
    const production_orders_update_data = {
        ...
        href_pageName : `production_orders_view_ar`,
        href_pageTitle : 'أوامر التصنيع',
...
document.addEventListener("DOMContentLoaded", async function () {
    try {
        ...
        if (conditionsArray.length === 0){
            ...
            back_title_page = 'إدارة التصنيع'
    
            pagePermission("view", permissionName);  // معلق
            sessionStorage.removeItem('production_orders_viewArray');
            backUp_page1(`production_orders_viewArray`, Qkey, permissionName, start_date, end_date, back_href_page, back_title_page)
            await restore_page1(getData_fn, `production_orders_viewArray`)
        } else {
            await restore_page1(getData_fn, `production_orders_viewArray`)
        }
