...
function backUp_filter_div_conditions() {
    ...
    const conditionsArray = JSON.parse(sessionStorage.getItem('services_Array')) || [];

    // إضافة الكائن الجديد إلى المصفوفة
    conditionsArray.push(conditions);

    // حفظ المصفوفة المحدثة في sessionStorage
    sessionStorage.setItem('services_Array', JSON.stringify(conditionsArray));
}
...
back_href.onclick = async function (event) {
    ...
    const array = JSON.parse(sessionStorage.getItem(`services_Array`)) || [];
...
function restore_filter_div_conditions(NUM_ektp_rakm_el_restore_elyEnta3ayzTerg3oMnel2a5er_maslan_1_ya3nyLastRestore) {
    ...
    // استرجاع المصفوفة المحفوظة من sessionStorage
    let conditionsArray = JSON.parse(sessionStorage.getItem("services_Array")) || [];
...
function CheckUrlParams_salesInvoice_update_ar() {
    try {
        ...
        if (!urlData || urlData.pageName !== "services_update_ar") {
            return true;
        }
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    await getData_fn();
    const conditionsArray = sessionStorage.getItem(`services_Array`);
