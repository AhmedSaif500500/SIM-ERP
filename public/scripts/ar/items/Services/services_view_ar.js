...
//pagePermission("view", "services_permission");  // معلق
...
newBtn.onclick = function (){
    // sessionStorage.removeItem('services_update_data')
    window.location.href = "/services_add_ar";
  }
...
back_href.onclick = async function (event) {
    ...
    await back_href_fn1(getData_fn, `services_viewArray`, `services_view_ar`, `itemsMain_view_ar`)
};
...
async function Execution() {
    try {
        ...
        permissionName = 'services_permission'
        start_date = false
        end_date = false
        Qkey = false
        back_href_page = 'services_view_ar'
        back_title_page = 'أصناف خدمية'

        showFirst50RowAtTheBegening();
        backUp_page1(`services_viewArray`, Qkey, permissionName, start_date, end_date, back_href_page, back_title_page)
    } catch (error) {
        catch_error(error);
    } finally {
    ...
    try {
        ...
        filteredData_Array = data.filter((row) => {
            ...
            const isaccount_nameMatch =
                ...
                const isActiveceMatch = filterData_string_column_with_showAndHiddenCheckbox_with_only_select(f3_checkbox, f3_select, 'is_inactive', row);
...
function fillTable() {
    try {
        ...
        let style_accountNo = `display:${f1_checkbox.checked ? "table-cell" : "none" }; width: auto; white-space: nowrap; text-align: start`;
        let style_accountName = `display: table-cell; width: 100%; white-space: nowrap; text-align: start;`;
        let style_is_inactive = `display:${f3_checkbox.checked ? "table-cell" : "none" }; width: auto; white-space: nowrap; text-align: start`;
        ...
        let tableHTML = `<table id="review_table" class="review_table">
                        <thead>
                            <tr>
                                ...
                                <th style="${style_accountNo}">المعرف</th>
                                <th style="${style_accountName}">الخدمة</th>
                                <th style="${style_is_inactive}">الحاله</th>
        ...
        slice_array1.forEach((row) => {
            ...
            tableHTML +=
                     `<tr>
                        ...
                        <td style="${style_accountNo}" class="td_account_no">${row.account_no}</td>
                        <td style="${style_accountName}" class="td_account_name">${row.account_name}</td>
                        <td style="${style_is_inactive}"><span class="${activeClass} td_is_inactive">${row.is_inactive}</span></td>
                      </tr>`;
        });
        ...
        tableHTML += `
                    <tr class="table_totals_row">
                        ...
                        <td id="footer_style_accountNo" style="${style_accountNo}"></td>
                        <td id="footer_style_accountName" style="${style_accountName}"></td>
                        <td id="footer_style_is_inactive" style="${style_is_inactive}"></td>
                    </tr>
                </tbody>
            </table>`;
...
function performSearch() {
    try {
        ...
        array1 = filteredData_Array.filter((row) => {
            ...
            const s3 = performSearch_Row(f3_checkbox,"is_inactive",searchValue,row);
...
async function table_view_btn_fn(updateBtn) {
    ...
    const permission = await btn_permission('services_permission', 'view');
    ...
    const services_update_data = {
        x: row.querySelector(`.td_id`).textContent,
        href_pageName : `services_view_ar`,
        href_pageTitle : 'أصناف خدميه',
    };

    sessionStorage.removeItem('services_update_data')
    sessionStorage.setItem('services_update_data', JSON.stringify(services_update_data));                            
    window.location.href = `services_update_ar`;
} catch (error) {
    catch_error(error)
} finally {
...
document.addEventListener("DOMContentLoaded", async function () {
    try {
        ...
        let conditionsArray = JSON.parse(sessionStorage.getItem("services_viewArray")) || [];
        if (conditionsArray.length === 0){
            ...
            backUp_page1(`services_viewArray`, Qkey, permissionName, start_date, end_date, back_href_page, back_title_page)
            await restore_page1(getData_fn, `services_viewArray`)
        } else {
            await restore_page1(getData_fn, `services_viewArray`)
        }
