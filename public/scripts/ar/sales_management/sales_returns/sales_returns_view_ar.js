...
pagePermission("view", "sales_returns_permission");  // معلق
...
function backUp_filter_div_conditions() {
    ...
    const conditionsArray = JSON.parse(sessionStorage.getItem('sales_returns_Array')) || [];

    // إضافة الكائن الجديد إلى المصفوفة
    conditionsArray.push(conditions);

    // حفظ المصفوفة المحدثة في sessionStorage
    sessionStorage.setItem('sales_returns_Array', JSON.stringify(conditionsArray));
}
...
back_href.onclick = async function (event) {
    ...
    const array = JSON.parse(sessionStorage.getItem(`sales_returns_Array`)) || [];
...
function restore_filter_div_conditions(NUM_ektp_rakm_el_restore_elyEnta3ayzTerg3oMnel2a5er_maslan_1_ya3nyLastRestore) {
    ...
    // استرجاع المصفوفة المحفوظة من sessionStorage
    let conditionsArray = JSON.parse(sessionStorage.getItem("sales_returns_Array")) || [];
...
async function getData_fn() {
    try {
        ...
        data = await new_fetchData_postAndGet(
            "/get_sales_returns_Data_view",
            {start_date, end_date},
            "sales_returns_permission","view",
            15,
            false,'',
            false,
            true,content_space,
        ...
        let tableHTML = `<table id="review_table" class="review_table">
                        <thead>
                            <tr>
                                ...
                                <th style="${style_invoice_reference}">فاتورة مبيعات</th>
...
async function table_view_btn_fn(updateBtn) {
    ...
    const permission = await btn_permission("sales_returns_permission", "view");
    ...
    const sales_returns_update_data = {
    ...
    sessionStorage.setItem('sales_returns_update_data', JSON.stringify(sales_returns_update_data));                            
...
function CheckUrlParams_salesreturns_update_ar() {
    try {
        const urlData = getURLData(
            "data",
            "sales_returns_view_ar",
            "رابط غير صالح : سيتم اعادة توجيهك الى صفحة القيود اليومية"
        );

        if (!urlData || urlData.pageName !== "sales_returns_update_ar") {
            return true;
        }
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    const result2 = CheckUrlParams_salesreturns_update_ar();
    if (!result2) {
        return;
    }

    await getData_fn();
    const conditionsArray = sessionStorage.getItem(`sales_returns_Array`);
