                  ...
                  <!-- dropdown -->
                    <div class="dropdown_container_input_table" id="">
                      <div class="row h_full">
                        ...
                        <div class="dropdown_select_input_table" id=""  onclick="tableDropdownList(this, '${encodeURIComponent(JSON.stringify(dataArray))}', '${encodeURIComponent(JSON.stringify(DropDown_accounts_tableColumnsName))}', 'account_type', 'salesReturnsForm')"  style="min-width: 10rem;">
...
let salesInvoiceReferencesArray = [];

async function showsalesreturnsData(x, invoiceId, customer_id, type){

  try {
    let permType
    let url;
    if (type === 'invoice'){
      url = "/get_data_for_invoiceToreturns"
      permType = 'add'
    }else{
      url = "/get_data_for_sales_returns_update"
      permType = 'update'
    }
  ...
  Data = await new_fetchData_postAndGet(
    url,
    {x, invoiceId, customer_id},
    'sales_returns_permission', permType, // معلق
    15,
    false,false,
    true,
    false,false,
  ...
  salesInvoiceReferencesArray =  Data.salesInvoiceReferencesArray;
    ...
    create_drop_down_with_External_DataArray(`dropdown_div4`,salesInvoiceReferencesArray); selectedRow_dropdownDiv(`dropdown_div4`,salesInvoiceReferencesArray,invoice_id);
