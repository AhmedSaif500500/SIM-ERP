...
function fillTable() {
    try {
        ...
        slice_array1.forEach((row) => {
            ...
            columns.forEach(col => {
                if (col === "item_id"){
                    tableHTML += `<td style="${style_id}">${row[col]}</td>`;
                }else if (col === "account_name"){
                    tableHTML += `<td style="${style_account_name}">${row[col]}</td>`; 
                }else if (col === "total_balance"){
                    tableHTML += tdNumber(true,true,true,row[col],deafult_style_balance,false,false)                
                }else{
                    let fn1 = `onclick = "table_balance1_btn_to_statetment_fn1(this, 'td_id', 'items_permission', firstDayOfYear, lastDayOfYear, 'items_table_view_ar', 'أصناف المخزون', {item_location_name : '${col}'}, 'item_movement_view_ar', 'obj_item_movement')"`;
